      ...
      <div className="flex flex-col group cursor-pointer h-full" style={{ background: CHAR, border: "1px solid rgba(255,255,255,.07)" }}
        ...
        <div className="p-6 flex flex-col flex-1">
          <div className="flex items-center gap-3 mb-3">
            <span style={{ fontFamily: "'Inter',sans-serif", fontSize: 11, color: "rgba(255,255,255,.35)" }}>{article.date}</span>
            <span style={{ width: 3, height: 3, borderRadius: "50%", background: "rgba(255,255,255,.2)", flexShrink: 0 }} />
            <span style={{ fontFamily: "'Inter',sans-serif", fontSize: 11, color: "rgba(255,255,255,.35)" }}>{article.readTime}</span>
          </div>
          <h3 style={{ fontFamily: "'Articulat CF',sans-serif", fontWeight: 700, fontSize: 18, color: "#fff", lineHeight: 1.25, marginBottom: 10, flex: 1 }}>
            {article.title}
          </h3>
          <p style={{ fontFamily: "'Inter',sans-serif", fontSize: 13, color: "rgba(255,255,255,.45)", lineHeight: 1.65, marginBottom: 18 }}>
            {article.desc}
          </p>
          <div className="flex items-center gap-2 self-start group/btn">
            <span style={{ fontFamily: "'Inter',sans-serif", fontWeight: 600, fontSize: 12, color: SAND, letterSpacing: 0.5 }}>Read more</span>
...
function DarkHorizontalCard({ article, onNavigate }: { article: Article; onNavigate?: (p: string) => void }) {
  return (
    <div className="flex gap-0 group cursor-pointer" style={{ background: CHAR, border: "1px solid rgba(255,255,255,.07)" }}
      onClick={() => onNavigate?.("blog-inner")}>
      ...
      <div className="flex flex-col justify-center px-5 py-5 flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-2">
          <span style={{ fontFamily: "'Articulat CF',sans-serif", fontWeight: 600, fontSize: 9, color: SAND, letterSpacing: 2, textTransform: "uppercase" }}>{article.cat}</span>
          <span style={{ width: 2, height: 2, borderRadius: "50%", background: "rgba(255,255,255,.2)", flexShrink: 0 }} />
          <span style={{ fontFamily: "'Inter',sans-serif", fontSize: 10, color: "rgba(255,255,255,.35)" }}>{article.readTime}</span>
        </div>
        <h4 style={{ fontFamily: "'Articulat CF',sans-serif", fontWeight: 700, fontSize: 15, color: "#fff", lineHeight: 1.3, marginBottom: 10 }}>
          {article.title}
        </h4>
        <div className="flex items-center gap-1.5 group/btn">
          <span style={{ fontFamily: "'Inter',sans-serif", fontWeight: 600, fontSize: 11, color: SAND }}>Read</span>
        ...
        {CATEGORIES.map((cat) => (
          <button key={cat} onClick={() => onChange(cat)}
            ...
            style={{
              ...
              color: active === cat ? DARK : "rgba(255,255,255,.5)",
              background: active === cat ? SAND : "transparent",
              border: `1px solid ${active === cat ? SAND : "rgba(255,255,255,.1)"}`,
              cursor: "pointer",
            }}>
            {cat}
          </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          ...
          <Reveal className="lg:col-span-7">
            <div className="relative overflow-hidden group cursor-pointer flex flex-col lg:h-[560px]"
              style={{ background: CHAR }} onClick={() => onNavigate("blog-inner")}>
              <div className="relative overflow-hidden flex-1" style={{ minHeight: 280 }}>
                ...
                <div className="absolute top-5 left-5">
                  <span className="px-3 py-1"
                    style={{ background: "rgba(196,171,108,.2)", border: "1px solid rgba(196,171,108,.45)", fontFamily: "'Articulat CF',sans-serif", fontWeight: 600, fontSize: 9, color: SAND, letterSpacing: 2.5, textTransform: "uppercase" }}>
                    {FEATURED_ARTICLES[0].cat}
                  </span>
                </div>
                <div className="absolute bottom-0 left-0 right-0 p-7">
                  <p style={{ fontFamily: "'Inter',sans-serif", fontSize: 11, color: "rgba(255,255,255,.4)", marginBottom: 10 }}>{FEATURED_ARTICLES[0].date} · {FEATURED_ARTICLES[0].readTime}</p>
                  <h3 style={{ fontFamily: "'Articulat CF',sans-serif", fontWeight: 800, fontSize: "clamp(24px,3vw,36px)", color: "#fff", lineHeight: 1.1, letterSpacing: "-0.5px", marginBottom: 10 }}>
                    {FEATURED_ARTICLES[0].title}
                  </h3>
                  <p style={{ fontFamily: "'Inter',sans-serif", fontSize: 14, color: "rgba(255,255,255,.55)", lineHeight: 1.65 }}>
                    {FEATURED_ARTICLES[0].desc}
                  </p>
                </div>
              </div>
              <div className="flex items-center justify-between px-7 py-4" style={{ borderTop: "1px solid rgba(255,255,255,.07)" }}>
                <div className="flex items-center gap-2 group/btn">
                  <span style={{ fontFamily: "'Inter',sans-serif", fontWeight: 600, fontSize: 13, color: SAND }}>Read article</span>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" className="transition-transform group-hover/btn:translate-x-0.5">
                    <path d="M5 12h14M13 6l6 6-6 6" stroke={SAND} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </div>
      ...
      <div className="max-w-[1440px] mx-auto px-8 md:px-14">
        <Reveal className="flex items-end justify-between gap-4 mb-12 flex-wrap">
          <div>
            <div className="flex items-center gap-3 mb-4">
              <div style={{ width: 8, height: 2, background: SAND }} />
              <span style={{ fontFamily: "'Articulat CF',sans-serif", fontWeight: 600, fontSize: 10, color: SAND, letterSpacing: 3.5, textTransform: "uppercase" }}>Articles</span>
            </div>
            <h2 style={{ fontFamily: "'Articulat CF',sans-serif", fontWeight: 800, fontSize: "clamp(28px,3.5vw,46px)", color: "#fff", lineHeight: 1.0, letterSpacing: "-1px" }}>
              More from our blog
            </h2>
          </div>
        ...
        <div className="flex items-center justify-center gap-2">
          <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
            ...
            style={{ border: "1px solid rgba(255,255,255,.15)", background: "none", cursor: "pointer" }}>
          ...
          {Array.from({ length: totalPages }).map((_, i) => (
            <button key={i} onClick={() => setCurrentPage(i + 1)}
              ...
              style={{ fontFamily: "'Inter',sans-serif", fontSize: 13, fontWeight: currentPage === i + 1 ? 600 : 400, color: currentPage === i + 1 ? DARK : "rgba(255,255,255,.5)", background: currentPage === i + 1 ? SAND : "none", border: `1px solid ${currentPage === i + 1 ? SAND : "rgba(255,255,255,.15)"}`, cursor: "pointer" }}>
              {i + 1}
            </button>
          ))}
...
function ReviewsSection() {
  ...
  return (
    <section className="py-20 lg:py-28 overflow-hidden" style={{ background: CHAR }}>
      <div className="max-w-[1440px] mx-auto px-8 md:px-14">
        <Reveal className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12">
          <div>
            <div className="flex items-center gap-3 mb-4">
              <div style={{ width: 8, height: 2, background: SAND }} />
              <span style={{ fontFamily: "'Articulat CF',sans-serif", fontWeight: 600, fontSize: 11, color: SAND, letterSpacing: 4, textTransform: "uppercase" }}>Reviews</span>
            </div>
            <h2 style={{ fontFamily: "'Articulat CF',sans-serif", fontWeight: 800, fontSize: "clamp(32px,4vw,52px)", color: "#fff", lineHeight: 1.0, letterSpacing: "-1px" }}>
              What our customers say
            </h2>
            <p style={{ fontFamily: "'Inter',sans-serif", fontSize: 15, color: "rgba(255,255,255,.45)", marginTop: 10, maxWidth: 400, lineHeight: 1.65 }}>
          ...
          {TESTIMONIALS.map((t) => (
            <div key={t.name} className="shrink-0 w-[min(85vw,500px)] flex flex-col"
              ...
              <div className="p-7 flex flex-col flex-1">
                ...
                <div style={{ fontFamily: "Georgia,serif", fontSize: 42, color: `rgba(196,171,108,.2)`, lineHeight: .7, marginBottom: 8 }}>&ldquo;</div>
                <p style={{ fontFamily: "'Inter',sans-serif", fontSize: 15, color: "rgba(255,255,255,.75)", lineHeight: 1.75, flex: 1, marginBottom: 20 }}>{t.quote}</p>
                <div className="flex items-center gap-3 pt-4" style={{ borderTop: "1px solid rgba(255,255,255,.07)" }}>
                  <div className="w-8 h-8 flex items-center justify-center shrink-0"
                    ...
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2" stroke="rgba(255,255,255,.6)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /><circle cx="12" cy="7" r="4" stroke="rgba(255,255,255,.6)" strokeWidth="1.8" /></svg>
                  </div>
                  <div>
                    <p style={{ fontFamily: "'Inter',sans-serif", fontWeight: 600, fontSize: 13, color: "#fff" }}>{t.name}</p>
                    <p style={{ fontFamily: "'Inter',sans-serif", fontSize: 11, color: "rgba(255,255,255,.4)" }}>{t.loc}</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="flex justify-center gap-2 mt-8 px-8">
        {TESTIMONIALS.map((_, i) => (
          <button key={i} onClick={() => emblaApi?.scrollTo(i)}
            ...
            style={{ width: cur === i ? 24 : 8, height: 8, background: cur === i ? SAND : "rgba(255,255,255,.2)", border: "none", cursor: "pointer", borderRadius: 4 }} />
        ))}
      </div>
    </section>
  );
