...
// ─── ProjectGalleryPage ──────────────────────────────────────────────────────────
// The standalone "interna" the "View all projects" link across the site was
// supposed to lead to. Reuses the exact same ProjectGallery component/data
// as the teaser sections (ResourcesPage, ServicePage) — same filter chips,
// same lightbox — just with `limit={Infinity}` so every matching project
// renders instead of the 6-card teaser.
export default function ProjectGalleryPage({ onBack, onNavigate }: { onBack: () => void; onNavigate?: (p: string) => void }) {
  return (
    <>
      ...
      <div className="w-full min-h-screen pt-[89px] md:pt-[123px] lg:pt-[139px] xl:pt-[155px]" style={{ background: SURFACE.base }}>
        ...
        <ProjectGallery id="gallery" limit={Infinity} onNavigate={onNavigate} />
